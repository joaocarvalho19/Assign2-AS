...
public class PProducer extends Thread{
    ...
    public PProducer(int id) {
        SERVER_PORT = PORT + id;
        
        //Open Gui
        prodGUI = new PProducerGUI();
        prodGUI.setVisible(true);
        prodGUI.updateTitle(id+1);
        ...
        properties.put("acks", "1"); // reduce lost records
        /* 
            Improve throughput
        */
        properties.put("linger.ms", 100);  // Number of milliseconds a producer is willing to wait before sending a batch out.
        properties.put("batch.size", 130000);    // It controls how many bytes of data to collect before sending messages to the Kafka broker.
        properties.put("compression.type", "lz4");   // The compression for all data generated by the producer. 
        properties.put("retries", 0);   // How many times the producer will attempt to send a message before marking it as failed.
    ...
    public void sendToTopic(String record){
        try{
            ...
            ProducerRecord<String, String> rec = new ProducerRecord<>(TOPIC, record);   // Key: order | value: rec
