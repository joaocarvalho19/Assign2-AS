...
public class PProducer extends Thread{
    ...
    public PProducer(int id) {
        ...
        /* 
            Minimize latency
        */
        properties.put("linger.ms", 0);  // Number of milliseconds a producer is willing to wait before sending a batch out.
        properties.put("batch.size", 16384);    // It controls how many bytes of data to collect before sending messages to the Kafka broker.
    ...
    public void sendToTopic(String record){
        try{
            String sensorID = record.split("ID:")[1].split(" ")[0];
            
            ProducerRecord<String, String> rec = new ProducerRecord<>(TOPIC, id, "Data", record);   // Partition | Key: order | value: rec
            System.out.println("++++Producer - "+record+" ID: "+id+"-"+Integer.parseInt(sensorID));
            producer.send(rec);
